...
TEST(highwayhash, main)
{
  uint8_t data[kMaxSize + 1] = {0};
  int i;
  for (i = 0; i <= kMaxSize; i++) {
    data[i] = i;
    uint64_t hash = HighwayHash64(data, i, kTestKey1);
    ASSERT_EQ(kExpected64[i], hash);
  ...
  uint64_t hash = HighwayHash64(data, 33, kTestKey2);
  ASSERT_EQ(0x53c516cce478cad7ull, hash);
}
